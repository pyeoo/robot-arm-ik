...
A real-time simulator of an articulated robotic arm in 3D, built to explore
forward and inverse kinematics from first principles. Drag a target and the arm
solves, joint by joint, how to reach it, smoothly easing into position while
respecting real joint angle limits, rendered as solid, lit geometry with real
depth and camera perspective.

The project started as a 2D prototype to validate the core kinematics and easing
logic with simpler math, then was extended into full 3D. Once the 3D
implementation was complete and verified, the original 2D components were
removed entirely rather than kept alongside it, so the codebase reflects a single,
current implementation instead of two overlapping ones.
...
- **Inverse kinematics (target-seeking mode)**: drag the target and the arm
  ...
  snap.
- **Solid, lit 3D rendering**: the arm and target render as real box geometry with
  per-face lighting and distance-based fog, not a wireframe, so depth and
  orientation are visually unambiguous rather than relying on foreshortening
  alone.
...
Since each joint is a hinge that can only rotate about one fixed axis, both
direction vectors are first projected onto the plane perpendicular to that
joint's world-space axis, and the signed angle around that specific axis is
recovered using a cross-product/dot-product formulation. A plain vector-angle
calculation has no concept of a specific rotation axis and can't produce a
signed, axis-relative result on its own, so this projection step is what makes
CCD work correctly in 3D rather than just in a single plane.
...
## Working in 3D: rotation, axis choice, and rendering depth

Three ideas needed genuinely different handling in 3D than a simpler 2D version
would:

- **Rotations are composed with quaternions, not angle addition.** 2D rotations
  commute (order doesn't matter), so cumulative rotation is just adding angles.
...
- **Every joint is a hinge with an explicit axis, and that axis choice matters.**
  An axis parallel to the joint's own link direction produces a joint that only
  rolls in place with no bending at all, and axes shared between two consecutive
  joints confine the whole sub-chain to a single bending plane. Getting genuinely
  non-planar 3D motion required each joint's axis to be distinct from both its own
  link direction and its neighbor's effective axis, two separate, easy-to-miss
  degenerate cases.
- **The renderer needed real depth cues, not just a 3D-looking projection.** A
...
- **`Kinematics3D`**, pure math. Forward kinematics derives joint positions and
  per-link transforms from a chain's current angles; `SolveCCD` iteratively
  adjusts a chain's angles to reach a target. Neither touches OpenGL or holds
  state, both are plain, testable transformations of data.
- **`Renderer3D`**, pure presentation. Draws whatever it's handed as lit solid
...
current angles every frame. This was a deliberate choice from the start: it
trades a small amount of redundant computation for eliminating an entire category
of "stale position" bugs where rendered state could drift out of sync with the
actual joint angles, and it's exactly what makes per-frame easing and re-solving
work correctly with no extra bookkeeping.
